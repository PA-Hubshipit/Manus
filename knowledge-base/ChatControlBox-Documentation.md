...
5. [Responsiveness](#5-responsiveness)
   - 5.6 [Proportionality System](#56-proportionality-system)
...
### 5.6 Proportionality System

The ChatControlBox implements a **proportionality system** that ensures all visual elements maintain balanced visual weight relative to each other. This system prevents proportionality drift when the component is modified or resized.

#### 5.6.1 The Proportionality Rule

> **CRITICAL:** All dimensions in ChatControlBox derive from a single master dimension (`--ccb-row-height`). To resize the entire component, change ONLY this value. Never use arbitrary pixel values in the component.

This rule ensures that:
- All elements scale proportionally when the component is resized
- Visual weight relationships are preserved across all viewport sizes
- Future modifications cannot accidentally break the design balance

#### 5.6.2 Configuration Files

The proportionality system consists of two configuration files:

| File | Purpose |
|------|--------|
| `client/src/config/chatcontrolbox.config.ts` | TypeScript config with master dimension, ratios, and computed values |
| `client/src/styles/chatcontrolbox.css` | CSS variables derived from the config |

#### 5.6.3 Master Dimension

The master dimension is the **Row Height** set at 48px. All other dimensions are calculated as ratios of this value:

```typescript
export const MASTER_ROW_HEIGHT = 48; // pixels
```

#### 5.6.4 Ratio Table

The following table defines the visual weight of each element relative to the master row height:

| Element | Ratio | Computed (px) | Notes |
|---------|-------|---------------|-------|
| **Row Height (master)** | 1.0 | 48px | Base unit for all calculations |
| **Toolbar Icon** | 0.5 | 24px | Menu, Plus, Bot, Settings, Save icons |
| **Input Icon** | 0.458 | 22px | Paperclip, Mic icons (slightly smaller) |
| **Send Icon** | 0.417 | 20px | Arrow up icon |
| **Toolbar Icon Button** | 0.833 | 40px | Clickable area for icon buttons |
| **Models Button Height** | 0.833 | 40px | Blue pill button |
| **Models Button Min Width** | 2.083 | 100px | Minimum width for text |
| **Presets Button Height** | 0.833 | 40px | Gray pill button |
| **Presets Button Min Width** | 1.875 | 90px | Minimum width for text |
| **Send Button** | 0.75 | 36px | Circular send button |
| **Toolbar Gap** | 0.25 | 12px | Gap between toolbar items |
| **Row Gap** | 0.25 | 12px | Gap between toolbar and input rows |
| **Container Padding** | 0.333 | 16px | Left/right padding |
| **Container Padding Y** | 0.25 | 12px | Top/bottom padding |
| **Input Height** | 0.917 | 44px | Text input field height |
| **Input Border Radius** | 0.458 | 22px | Full rounded (half of height) |
| **Container Border Radius** | 0.333 | 16px | Corner radius |
| **Font Size** | 0.292 | 14px | Button text size |
| **Min Container Width** | - | 362px | Calculated from all toolbar items |

#### 5.6.5 CSS Variables

The CSS variables are applied to the component root via the `.chat-control-box` class:

```css
.chat-control-box {
  --ccb-row-height: 48px;
  --ccb-toolbar-icon-size: calc(var(--ccb-row-height) * 0.5);
  --ccb-toolbar-icon-button-size: calc(var(--ccb-row-height) * 0.833);
  /* ... additional variables ... */
}
```

#### 5.6.6 Usage in Component

The component uses CSS classes that reference these variables:

```tsx
// Toolbar icon button
<button className="ccb-toolbar-icon-button text-zinc-400 hover:text-white">
  <Menu className="ccb-toolbar-icon" />
</button>

// Models button (blue pill)
<button className="ccb-models-button bg-blue-500 text-white">
  {selectedModels.length} Models
</button>
```

#### 5.6.7 Minimum Width Constraint

The component calculates a minimum container width to ensure all toolbar items fit on one row:

```
Min Width = (5 × Icon Button) + Models Button + Presets Button + (6 × Gap) + (2 × Padding)
         = (5 × 40px) + 100px + 90px + (6 × 12px) + (2 × 16px)
         = 200px + 100px + 90px + 72px + 32px
         = 362px
```

If the container is narrower than 362px, the toolbar items will wrap or compress, breaking the proportional layout.

#### 5.6.8 Modification Guidelines

When modifying the ChatControlBox:

1. **Never use arbitrary pixel values** - Always use CSS variables or ratios
2. **To resize the component** - Change only `MASTER_ROW_HEIGHT` in the config
3. **To add new elements** - Define their ratio relative to row height
4. **To change proportions** - Update the ratio in the config, not the component
